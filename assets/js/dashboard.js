...
function renderDashboard(){
  ...
  root.innerHTML=`<section class="dashboard-section"><h2>This week’s releases</h2><p class="dashboard-note">Today and the next six days. Times are local; broadcast and listed film dates may differ from UK streaming availability.</p>${days}${unknown.length?`<details><summary>Dates to be announced (${unknown.length})</summary>${unknown.map(item=>`<button type="button" class="dashboard-title" data-dashboard-open="${esc(item.id)}">${esc(item.title)} · Date TBA</button>`).join('')}</details>`:''}</section><section class="dashboard-section"><h2>Next to watch</h2>${next.length?next.slice(0,5).map(row).join(''):'<p class="dashboard-note">No confirmed released episodes left to watch. Schedules refresh automatically.</p>'}${next.length>5?`<details><summary>Show ${next.length-5} more</summary>${next.slice(5).map(row).join('')}</details>`:''}</section>`;
}
